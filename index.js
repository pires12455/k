...
conn.sendMessage(id, 'Owner Pires wa.me/+556996103915' ,MessageType.text);
}
else if (text == 'affis'){
  ...
  var teks = text.replace(/#nulis /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, '[WAIT] espere um pouco brow...🔍', MessageType.text)
  ...
  var teks = text.replace(/#texthunder /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, '🔍 SENDO PROCESSADO', MessageType.text)
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, buf, MessageType.image)
        })
    })
    ...
    axios.get(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${bg}`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ WAIT ] Em andamento🔍 por favor, aguarde um momento', MessageType.text, { quoted: m })
...
axios.get(`https://api.haipbis.xyz/harinasional?tanggal=${teks}`).then((res) => {
    let hasil = `➸  *Encontro : ${res.data.tanggal}*\n\n➸ em formação : ${res.data.keterangan}`;
    conn.sendMessage(id, hasil ,MessageType.text);
  })
 }
...
conn.sendMessage(id, 'Grupo de Mudança de Nome de Sucesso' ,MessageType.text, { quoted: m } );
...
conn.sendMessage(id, 'Grupo de Descrição de Mudança de Sucesso' ,MessageType.text, { quoted: m } );

}
if (text.includes('#join')){
conn.sendMessage(id, {displayname: "Jeff", vcard: vcard}, MessageType.contact)
conn.sendMessage(id, 'Quer doar para inserir o Bot no grupo? Proprietário do bate-papo :D', MessageType.text)
}
if (text.includes("#bitly")){
const teks = text.replace(/#bitly /, "")
axios.get(`https://api.haipbis.xyz/bitly?url=${teks}`).then((res) => {
	conn.sendMessage(id, '[WAIT] Procurando...🔍', MessageType.text)
    let hasil = `nih kak :) \n\n${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
conn.sendMessage(id, 'SUCESSOS, O GRUPO ABRIU ' ,MessageType.text, { quoted: m } );
}
else if (text == '#closegc'){
 let hasil = `${id.split("@s.whatsapp.net")[0]}`;
   conn.groupSettingChange (hasil, GroupSettingChange.messageSend, true);
conn.sendMessage(id, 'SUCESSOS, O GRUPO FECHOU' ,MessageType.text, { quoted: m } );
}
...
axios.get(`https://arugaz.herokuapp.com/api/getzodiak?nama=aruga&tgl-bln-thn=${teks}`).then((res) => {
    let hasil = `🔍Nascermos : ${res.data.lahir}*\n🔍aniversário : ${res.data.ultah}\n🔍idade : ${res.data.usia}\n🔍 Zodíaco : ${res.data.zodiak}`;
    conn.sendMessage(id, hasil ,MessageType.text);
  })
 }
...
axios.get(`https://arugaz.herokuapp.com/api/howgay`).then((res) => {
	conn.sendMessage(id, '[WAIT] Processar...🔍', MessageType.text)
    let hasil = ` ${res.data.desc} \n\n *Porcentagem Gay Lo!!!* _${res.data.persen}_`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://mhankbarbar.herokuapp.com/api/twit?url=${teks}&apiKey=zFuV88pxcIiCWuYlwg57`).then((res) => {
	conn.sendMessage(id, '[WAIT] Procurando...🔍', MessageType.text)
    let hasil = `🔍Funciona$ por favor clique no link abaixo para baixar os resultados$\nClique no link abaixoðŸ—¡ï¸\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://arugaz.herokuapp.com/api/fb?url=${teks}`).then((res) => {
    let hasil = `Download você mesmo através do link abaixo, infelizmente o servidor está fora do ar xixi..\n\nJudul: ${res.data.title}\n\nSize: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://masak-apa.tomorisakura.vercel.app/api/search/?q=${teks}`).then((res) => {
	conn.sendMessage(id, '[WAIT] Procurando...🔍', MessageType.text)
    let hasil = ` *Título:* ${results.title}\n*Autor:* ${results.author.user}\n*Rilis:* ${results.author.datePublished}\n*Level:* ${results.dificulty}\n*Tempo:* ${results.times}\n*Parte:* ${results.servings}\n\n*Bahan-bahan:*\n${bahan}\n\n*Step-by-step:*\n${tutor}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
const get = require('got')
    ...
    conn.sendMessage(id,`🔎ÚLTIMOS DADOS DO DISTRITO COVID-19 DA INDONÉSIA🔍\n\n📈Positivo ==> ${positif} \n📉Ser curado ==> ${sembuh} \n📋Morreu ==> ${meninggal}\n🗒️Ser tratado ==> ${dirawat}`, MessageType.text);
}
...
🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍🔍
   ...
   {
    var items = ["menina ulzzang "," menina bonita "," hijab lindo "," menina coreana "," adolescente bonita "," menina coreana "," menina japonesa "];
   ...
   {
    var items = ["cara bonito "," cogan "," garoto coreano "," garoto chinês "," garoto japonês "," garoto indo bonito "," garoto coreano "];
...
if (text.includes("#fotoanime"))
   {
    var items = ["anime girl "," anime cantik "," anime "," anime estética "," anime hd "," gambar anime hd "];
    var nime = items[Math.floor(Math.random() * items.length)];
    var url = "https://api.fdci.se/rep.php?gambar=" + nime;
...
if (text.includes('#infogempa')){
    axios.get(`https://st4rz.herokuapp.com/api/infogempa`).then((res) => {
      imageToBase64(res.data.map)
        .then(
          (ress) => {
          let hasil = `*ÚLTIMAS INFORMAÇÕES SOBRE O TERREMOTO NA INDONÉSIA *\n\n*➸ Centro de Terremotos :* ${res.data.lokasi}\n*➸ Coordenada :* ${res.data.koordinat}\n*➸ Waktu :* ${res.data.waktu}\n*➸ Magnitudo :* ${res.data.magnitude}\n*➸ Kedalaman :* ${res.data.kedalaman}\n*➸ Potensi :* ${res.data.potensi}`;
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ WAIT ] Em andamento🔍 por favor, aguarde um momento', MessageType.text, { quoted: m })
            conn.sendMessage(id, buf, MessageType.image, { caption: hasil, quoted: m })
        })
